...
export function Settings({ api, settings, onChanged }: { api: PCWrappedAPI; settings: TrackingSettings; onChanged: () => void }) {
  ...
  return <motion.div className="page settings-page" initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }}>
    ...
    <section className="settings-columns"><div>
      <article className="settings-group"><header><ShieldCheck /><div><h2>Tracking & privacy</h2><p>Foreground apps only. Never keystrokes or screen contents.</p></div></header>
        <SettingRow icon={<Power />} title="Background tracking" copy="Remember the foreground app while PC Wrapped is closed." control={<button className={`switch ${settings.trackingEnabled ? 'is-on' : ''}`} onClick={() => update({ trackingEnabled: !settings.trackingEnabled })}><i /></button>} />
      ...
      <article className="settings-group"><header><Info /><div><h2>Desktop behavior</h2><p>How PC Wrapped lives alongside everything else.</p></div></header>
