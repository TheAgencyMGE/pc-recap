...
export function Achievements({ achievements }: { achievements: Achievement[] }) {
  ...
  return <motion.main className="page achievements-page" initial={reduceMotion ? false : { opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }}>
    ...
    <section className="achievement-grid">{achievements.map((item, index) => {
      ...
      return <motion.article key={item.id} className={`achievement ${done ? 'is-unlocked' : ''}`} style={{ '--achievement-color': item.accent } as React.CSSProperties} initial={reduceMotion ? false : { opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: reduceMotion ? 0 : index * .05 }} data-record-state={done ? 'unlocked' : 'locked'}>
