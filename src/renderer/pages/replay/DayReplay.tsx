import { Clock3, GitCompareArrows, LoaderCircle, MoonStar, Sparkles } from 'lucide-react';
...
export function DayReplay({ api, day }: { api: PCRecapAPI; day: string }) {
  ...
  return <motion.main className="page replay-page" initial={false} animate={{ opacity: 1 }}>
    <header className="replay-heading"><div><small>Day Replay</small><h1>{formatDate(`${day}T12:00:00`)}</h1></div><strong>{formatDurationLong(data.totalSeconds)}</strong></header>
    ...
    {(data.recoveredClues.length > 0 || data.pins.length > 0) && <section className="replay-notes"><h2>Along the way</h2>{data.pins.map((pin) => <article key={pin.id}><b>{pin.title}</b><span>{pin.note}</span></article>)}{data.recoveredClues.map((clue) => <article key={clue.id}><b>{clue.appName}</b><span>{clue.detail ?? clue.eventType}</span></article>)}</section>}
  </motion.main>;
}
