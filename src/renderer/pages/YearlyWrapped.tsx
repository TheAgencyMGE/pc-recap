...
import { buildWrappedScenes, formatWrappedTotal, type WrappedSceneId } from '../lib/wrapped-scenes';
...
function SceneKicker({ children }: { children: React.ReactNode }) {
  return <span className="wrapped-kicker">{children}</span>;
}

function Scene({ id, summary, timeline, api }: { id: WrappedSceneId; summary: PeriodSummary; timeline: TimelineBucket[]; api: PCWrappedAPI }) {
  ...
  const total = formatWrappedTotal(summary.totalSeconds);

  if (id === 'cover') return (
    <section className="wrapped-scene wrapped-scene--cover">
      <h1>This was your <em>{summary.label}.</em></h1>
      <div className="wrapped-cover-mark" aria-hidden="true"><span>PC</span><b>{summary.label}</b><i>WRAPPED</i></div>
    </section>
  );

  if (id === 'total') return (
    <section className="wrapped-scene wrapped-scene--total">
      ...
      <div className="wrapped-time-rule" aria-hidden="true"><i /><i /><i /><i /><i /></div>
    </section>
  );

  if (id === 'favorite' && favorite) return (
    <section className="wrapped-scene wrapped-scene--favorite">
      <div className="wrapped-favorite-copy">
      ...
      <div className="wrapped-favorite-art">
        <AppIcon appId={favorite.appId} name={favorite.name} color={favorite.color} size="large" />
        <span>NO. 1</span>
      </div>
      {summary.topApps.length > 1 && <ol className="wrapped-mini-chart" aria-label="Most-used apps">
  ...
  if (id === 'categories') return (
    <section className="wrapped-scene wrapped-scene--categories">
      <SceneKicker>Categories</SceneKicker>
      <h1>Your year had range.</h1>
      <div className="wrapped-category-stack" aria-label="Category usage">
  ...
  if (id === 'sprint' && busiestMonth) return (
    <section className="wrapped-scene wrapped-scene--sprint">
      ...
      <div className="wrapped-months" aria-label="Recorded months">
  ...
  if (id === 'era' && era) return (
    <section className="wrapped-scene wrapped-scene--era" style={{ '--era-color': era.color } as React.CSSProperties}>
      <SceneKicker>Era</SceneKicker>
      <h1>{era.title}</h1>
      <p>{era.subtitle}</p>
      <div className="wrapped-era-stamp"><span>{era.start}</span><b>ERA</b><span>{era.end}</span></div>
    </section>
  );

  if (id === 'memory' && observation) return (
    <section className="wrapped-scene wrapped-scene--memory" style={{ '--memory-color': observation.accent } as React.CSSProperties}>
      <SceneKicker>{observation.eyebrow}</SceneKicker>
      <h1>“{observation.text}”</h1>
      <p>{observation.detail}</p>
      <div className="wrapped-memory-ticket"><b>{summary.label}</b></div>
    </section>
  );

  if (id === 'record' && summary.longestSession) return (
    <section className="wrapped-scene wrapped-scene--record">
      ...
      <div className="wrapped-record-lines" aria-hidden="true"><i /><i /><i /></div>
    </section>
  );

  return (
    <section className="wrapped-scene wrapped-scene--finale">
      ...
      <button className="wrapped-share" onClick={async () => {
      ...
      <button className="wrapped-share wrapped-share--secondary" onClick={async () => {
        const result = await saveShareCard(summary, api, 'story');
        setSaved(result.ok ? 'Saved to your PC.' : result.canceled ? '' : result.error ?? 'Could not save.');
      }}><Download /> Story</button>
      </div>
      <p className="wrapped-saved" aria-live="polite">{saved}</p>
    </section>
  );
}

export function YearlyWrapped({ api, summary, timeline, onClose }: Props) {
  const sceneIds = useMemo(() => buildWrappedScenes(summary, timeline), [summary, timeline]);
  ...
  return (
    <div className={`yearly-wrapped yearly-wrapped--${sceneId}`}>
      <header className="wrapped-header">
        <button onClick={onClose}><X /><span>Exit story</span></button>
        <div className="wrapped-progress" aria-label="Story scenes">
