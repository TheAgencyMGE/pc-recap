...
export function buildWrappedScenes(summary: PeriodSummary, timeline: TimelineBucket[]): WrappedSceneId[] {
  const scenes: WrappedSceneId[] = ['cover', 'total'];
...
export function formatWrappedTotal(seconds: number) {
  if (seconds < 3_600) {
