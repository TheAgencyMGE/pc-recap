...
describe('YearlyWrapped', () => {
  it('moves through cinematic scenes with the keyboard', async () => {
    ...
    render(<AppIconProvider api={api}><YearlyWrapped api={api} summary={summary} timeline={timeline} onClose={() => undefined} /></AppIconProvider>);

    expect(screen.getByRole('heading', { name: new RegExp(`this was your ${year}`, 'i') })).toBeInTheDocument();
    expect(screen.queryByText(/private playback|annual edition|pcw/i)).not.toBeInTheDocument();
